...
class UserService:
    ...
    def nextPk(self):
        pk = 1  # Default value if no records exist
        with connection.cursor() as cursor:
            cursor.execute("SELECT MAX(id) FROM sos_user")
            result = cursor.fetchone()  # fetchone() is better than fetchall() for a single row
        if result and result[0]:  # Check if result is not None
            pk = result[0] + 1
        return pk  # Always return an integer
